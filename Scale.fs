...
let cassa (s: Stato) : int =
    s.movimenti |> List.map (contabile s) |> List.sum
...
let tesoretto (s: Stato)  : int =
    let altro = s.movimenti |> List.map (altroContabile s) |> List.sum
    let pagamenti = s.movimenti |> List.map (pagamentoScale s) |> List.sum
...
let quote (s: Stato) (c: Condomino) : int =
    let versamentiQuote =  s.movimenti |> List.map (versamentoQuote s c) |> List.sum
...
let prestito (s: Stato) : int =
     s.movimenti |> List.map  (prestitiContabile s) |> List.sum |> ( ~- )